...
static const uint16_t arm_indices[] = {
...
void draw_cube()
{
    ...
    glVertexPointer(            3, GL_FLOAT,            sizeof(skinned_vertex_t),    arm_skeletal[0].position);
    glTexCoordPointer(          2, GL_FLOAT,            sizeof(skinned_vertex_t),    arm_skeletal[0].texcoord);
    glNormalPointer(               GL_FLOAT,            sizeof(skinned_vertex_t),    arm_skeletal[0].normal);
    glMatrixIndexPointerARB(    1, GL_UNSIGNED_BYTE,    sizeof(skinned_vertex_t),    &arm_skeletal[0].mtx_index);

    glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(vertex_t), (void*)(8*sizeof(float) + arm_skeletal));

    glDrawElements(GL_TRIANGLES, sizeof(arm_skeletal) / sizeof(uint16_t), GL_UNSIGNED_SHORT, arm_skeletal);
...
void draw_skinned()
{
    ...
    glDrawArrays(GL_TRIANGLE_STRIP, 0, sizeof(arm_skeletal)/sizeof(skinned_vertex_t));
